...
- C-ABI key resolvers used by the save editor (Skill / Mission / Quest / Stage / Knowledge / QuestGauge / SubLevel / GimmickInfo / Character + checksum), plus three dye gamedata bridges (DyeColorGroupInfo / PartPrefabDyeTexturePallete / PartPrefabDyeSlot), three faction gamedata bridges (FactionNode / FactionSpawnData / FactionRelationGroup), an `iteminfo` socket / artifact-challenge / canonical-gem advisory surface, and save-handle mutation helpers (inventory enumeration, `mutation_version` for snapshot staleness, and a deferred-redecode batch ABI that suspends the per-call `decode_blocks` for bulk-mutation workflows — one decode at end of batch instead of N)
