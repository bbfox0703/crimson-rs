...
|---|---|---|---|
| `keys.txt` | Current-version Crimson Desert itemKey list (one decimal key per line, in in-game order). Tracks the latest game patch — currently **1.11** with 6,333 keys (1.11 added 8 items vs 1.10). | Dumped from `CrimsonDesert.exe` process memory via Cheat Engine (see Generation below) | ~54 KB |
| `keys-1.11.txt` | Snapshot dumped against the live 1.11 install. Byte-identical to `keys.txt` — kept so a future patch that *does* reorder keys can still be diffed back against the 1.11 baseline. | Frozen from the live 1.11 dump | ~54 KB |
