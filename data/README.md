...
|---|---|---|---|
| `keys.txt` | Current-version Crimson Desert itemKey list (one decimal key per line, in in-game order). Tracks the latest game patch — currently **1.12** with 6,483 keys (1.12 added 150 net items vs 1.11). | Dumped from `CrimsonDesert.exe` process memory via Cheat Engine (see Generation below) | ~55 KB |
| `keys-1.12.txt` | Snapshot dumped against the live 1.12 install. Byte-identical to `keys.txt` — kept so a future patch that *does* reorder keys can still be diffed back against the 1.12 baseline. | Frozen from the live 1.12 dump | ~55 KB |
