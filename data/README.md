...
|---|---|---|---|
| `keys.txt` | Current-version Crimson Desert itemKey list (one decimal key per line, in in-game order). Tracks the latest game patch — currently **2.01** with 6,813 keys (2.01 added 3 items — 1005600, 1005601, 1005852 — and removed none; key 1000319 also moved position). | Dumped from `CrimsonDesert.exe` process memory via Cheat Engine (see Generation below) | ~58 KB |
| `keys-2.01.txt` | Snapshot dumped against the live 2.01 install. Byte-identical to `keys.txt`. | Frozen from the live 2.01 dump | ~58 KB |
