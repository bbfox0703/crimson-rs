...
|---|---|---|---|
| `keys.txt` | Current-version Crimson Desert itemKey list (one decimal key per line, in in-game order). Tracks the latest game patch — currently **2.00** with 6,810 keys (2.00 added 237 items and removed none). | Dumped from `CrimsonDesert.exe` process memory via Cheat Engine (see Generation below) | ~58 KB |
| `keys-2.00.txt` | Snapshot dumped against the live 2.00 install. Byte-identical to `keys.txt` — kept so a future patch that *does* reorder keys can still be diffed back against the 2.00 baseline. | Frozen from the live 2.00 dump | ~58 KB |
