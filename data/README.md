...
|---|---|---|---|
| `keys.txt` | Current-version Crimson Desert itemKey list (one decimal key per line, in in-game order). Tracks the latest game patch — currently **1.17** with 6,572 keys (1.17 *removed* 9 items vs 1.16 and added none). | Dumped from `CrimsonDesert.exe` process memory via Cheat Engine (see Generation below) | ~56 KB |
| `keys-1.17.txt` | Snapshot dumped against the live 1.17 install. Byte-identical to `keys.txt` — kept so a future patch that *does* reorder keys can still be diffed back against the 1.17 baseline. | Frozen from the live 1.17 dump | ~56 KB |
