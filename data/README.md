...
|---|---|---|---|
| `keys.txt` | Current-version Crimson Desert itemKey list (one decimal key per line, in in-game order). Tracks the latest game patch — currently **2.02** with 6,813 keys, byte-identical to the 2.01 list (2.02 added and removed no items; 2.01 had added 3 — 1005600, 1005601, 1005852 — removed none, and moved key 1000319). | Dumped from `CrimsonDesert.exe` process memory via Cheat Engine (see Generation below) | ~58 KB |
| `keys-2.02.txt` | Snapshot dumped against the live 2.02 install. Byte-identical to `keys.txt` and to `keys-2.01.txt` (content-only patch, no item add/remove). | Frozen from the live 2.02 dump | ~58 KB |
