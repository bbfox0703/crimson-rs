...
| **1.11** | **6,333 keys** | [`keys.txt`](keys.txt) (current) + [`keys-1.11.txt`](keys-1.11.txt) snapshot. +8 keys vs 1.10. 1.11 ships one iteminfo schema drift (new boolean `u8` between `apply_drop_stat_type` and `drop_default_data`) and **no** save-body drift — see `src/item_info/item.rs`. |

The other 30 gamedata tables get the same treatment under [`gamedata-keys-1.08/`](gamedata-keys-1.08/), [`gamedata-keys-1.09/`](gamedata-keys-1.09/), [`gamedata-keys-1.10/`](gamedata-keys-1.10/) and [`gamedata-keys-1.11/`](gamedata-keys-1.11/) — per-version snapshots so the next patch's drift can be diffed table-by-table without needing the previous game install on disk. 1.11 deltas vs 1.10 (diff the dirs): gameplayvariableinfo +8, missioninfo +9, stageinfo +9, questinfo +1; all 26 other tables unchanged. 1.10 deltas vs 1.09: missioninfo +134, stageinfo +358, gimmickinfo +282, questinfo +65, knowledgeinfo +43, questgaugeinfo +43, itemgroupinfo +14, characterinfo/skill/factionspawndatainfo +8, gameadviceinfo +7, partprefabdyeslotinfo +6, regioninfo/storeinfo +1, factionnode −17.
