...
| **1.12** | **6,483 keys** | [`keys.txt`](keys.txt) (current) + [`keys-1.12.txt`](keys-1.12.txt) snapshot. +150 keys vs 1.11. 1.12 ships **four** iteminfo schema drifts (SubItem `type_id==16`, `unk_pre_max_endurance`, equipment/gem-gated `unk_pre_gimmick_visual`, `EnchantData` inter-element separators) and **no** save-body drift — see `src/item_info/item.rs`. |

The other 30 gamedata tables get the same treatment under [`gamedata-keys-1.08/`](gamedata-keys-1.08/), [`gamedata-keys-1.09/`](gamedata-keys-1.09/), [`gamedata-keys-1.10/`](gamedata-keys-1.10/) [`gamedata-keys-1.11/`](gamedata-keys-1.11/) and [`gamedata-keys-1.12/`](gamedata-keys-1.12/) — per-version snapshots so the next patch's drift can be diffed table-by-table without needing the previous game install on disk. 1.12 deltas vs 1.11 (diff the dirs): gimmickinfo +91, knowledgeinfo +72, storeinfo +53, stageinfo +37, itemgroupinfo +20, characterinfo +7, skill +5, reserveslot +4, crafttoolinfo/crafttoolgroupinfo/gameadviceinfo/questinfo +2 each, gameplayvariableinfo +1, partprefabdyeslotinfo −143, factionnode −22; all 15 other tables unchanged. 1.11 deltas vs 1.10: gameplayvariableinfo +8, missioninfo +9, stageinfo +9, questinfo +1; all 26 other tables unchanged. 1.10 deltas vs 1.09: missioninfo +134, stageinfo +358, gimmickinfo +282, questinfo +65, knowledgeinfo +43, questgaugeinfo +43, itemgroupinfo +14, characterinfo/skill/factionspawndatainfo +8, gameadviceinfo +7, partprefabdyeslotinfo +6, regioninfo/storeinfo +1, factionnode −17.
