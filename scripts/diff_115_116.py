"""Cross-version drift RE: 1.15 → 1.16 iteminfo schema.
...
parser handles 1.15 cleanly and fails on every 1.16 item, so the 1.15
side supplies the per-field span map and the 1.16 side is walked against
it byte-for-byte.

1.16 ships heavy *value* churn on top of the schema drift (e.g. the
`item_use_info_list` elements went from 0x7ffffffe-style hashes to
sequential 1,000,000+ ids), so same-length substitutions are common;
MAX_SHIFT tolerates them as `shift_old == shift_new` steps rather than
aborting the walk.

Inputs:
    out/iteminfo.1.15.pabgb   (previous baseline; override with $OLD_ITEMINFO)
    out/iteminfo.pabgb        (current 1.16 extract, written by export_for_ce.py)
    data/keys.txt             (current 1.16 key order, 6,581 keys)

Output: `out/diff_115_116/report.txt` — sorted by event signature so
schema changes show up as the top-frequency lines.

Usage:
    python scripts/diff_115_116.py
"""
...
OLD_PABGB = Path(
    os.environ.get("OLD_ITEMINFO", str(REPO / "out" / "iteminfo.1.15.pabgb"))
)
NEW_PABGB = REPO / "out" / "iteminfo.pabgb"
OUT_DIR = REPO / "out" / "diff_115_116"
...
def main():
    ...
    with report_path.open("w", encoding="utf-8") as fh:
        fh.write(
            f"1.15 -> 1.16 iteminfo drift report\n"
            f"{'=' * 60}\n"
            f"1.15 bytes: {len(old_bytes_full):,}  items: {len(old_items)}\n"
            f"1.16 bytes: {len(new_bytes_full):,}  keys: {len(new_keys)}  "
