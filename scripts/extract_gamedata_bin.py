...
Mirrors `<game>/0008/gamedata/binary__/client/bin/` (iteminfo + skill +
...
def list_bin_files(game_dir: str) -> list[str]:
    """Enumerate every file in 0008's gamedata/binary__/client/bin directory."""
    ...
    for d in pamt["directories"]:
        ...
        # normalize separators for the contains-check
        if BIN_DIR not in dpath.replace("\\", "/"):
            continue
...
def main() -> None:
    ...
    names = list_bin_files(game)
    print(f"{len(names)} files in {BIN_GROUP}/{BIN_DIR}")
    ...
    for name in names:
        try:
            raw = bytes(crimson_rs.extract_file(game, BIN_GROUP, BIN_DIR, name))
