"""Cross-version drift RE: 1.12 → 1.13 iteminfo schema.

Lightweight byte-diff workflow (no sibling parser install needed — the
in-tree parser handles 1.12 cleanly). For each item key present in both
1.12 and 1.13, walk per-byte through the old + new chunks side-by-side;
...
Field context: every drift event is tagged with the 1.12 field path
...
Inputs:
    X:\\Crimson Desert\\gamedata-bin\\1.12\\iteminfo.pabgb  (previous baseline)
    out/iteminfo.pabgb        (current 1.13 extract, written by export_for_ce.py)
    data/keys.txt             (current 1.13 key order, 6,508 keys)

Output: `out/diff_112_113/report.txt` — sorted by event signature so
schema changes show up as the top-frequency lines.

Usage:
    python scripts/diff_112_113.py
"""
...
OLD_PABGB = Path(
    os.environ.get("OLD_ITEMINFO", r"X:\Crimson Desert\gamedata-bin\1.12\iteminfo.pabgb")
)
NEW_PABGB = REPO / "out" / "iteminfo.pabgb"
OUT_DIR = REPO / "out" / "diff_112_113"
OUT_DIR.mkdir(parents=True, exist_ok=True)

MAX_SHIFT = 16  # brute-force shift range on each side at a mismatch (16 so a
#                 same-length value substitution up to 16 B is stepped over as
#                 so==sn rather than aborting the whole walk)
...
def field_at(spans_ranges: list[dict], rel_off: int) -> str:
    ...
    1.12 item span. If past the end, returns the LAST tracked path with a
...
def main():
    ...
    with report_path.open("w", encoding="utf-8") as fh:
        fh.write(
            f"1.12 -> 1.13 iteminfo drift report\n"
            f"{'=' * 60}\n"
            f"1.12 bytes: {len(old_bytes_full):,}  items: {len(old_items)}\n"
            f"1.13 bytes: {len(new_bytes_full):,}  keys: {len(new_keys)}  anchors_resolved: {resolved}\n"
    ...
    print(f"Top 12 signatures:")
    for sig, cnt in signature_count.most_common(12):
        try:
            print(f"  [{cnt:>5}x] {sig}")
