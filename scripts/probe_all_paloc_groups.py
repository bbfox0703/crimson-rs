"""Scan ALL groups (not just first hit) for a language's paloc data.
Crimson Desert ships patch data in higher group numbers, so a newer paloc
might live in a later group than the base one we hit first."""
...
from gamedata_layout import discover_paloc_targets
...
def main() -> None:
    ...
    print(f"scanning groups {lo}..{hi-1} for the {args.lang} paloc\n")
    ...
    for g in groups:
        # One group at a time — the whole point of this probe is which group
        # carries the newest copy, so it must not dedupe across them. 2.01
        # splits a language into one file per namespace; merge those.
        targets = [t for t in discover_paloc_targets(args.game_dir, [g])
                   if t.lang == args.lang]
        if not targets:
            continue
        t = targets[0]
        ...
        for fname in t.files:
            raw = bytes(crimson_rs.extract_file(args.game_dir, g, t.dir, fname))
            size += len(raw)
    ...
    if not hits_per_group:
        print(f"no group carries a {args.lang} paloc")
        return
