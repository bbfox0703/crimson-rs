...
5. **Save-side probes** (run only if downstream reports the editor breaking; not part of the routine green-light loop). Four `#[ignore]` diagnostics live in [`../src/c_abi/character_info.rs`](../src/c_abi/character_info.rs) and skip cleanly when their inputs are missing:
