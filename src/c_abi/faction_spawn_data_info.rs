//! `factionspawndatainfo.pabgb` bridge — C ABI surface.
//!
//! Resolves save-side `FactionSpawnDataKey (u32)` to the row's
//! internal name (e.g.
//! `"FactionSpawn_GlenbrightManor_Grace_ReedDevil"`). Mirrors
//! [`super::sub_level_info`] / [`super::quest_gauge_info`] / sibling
//! [`super::faction_node_info`] in shape — **no `lookup_display_name`
//! surface**, the PALOC probe surfaced only coincidental collisions.
//! See `src/faction_spawn_data_info/mod.rs` for schema notes.
...
use super::error;
use crate::faction_spawn_data_info::parse_faction_spawn_data_info_lossy;

/// Opaque handle exposing `(FactionSpawnDataKey, internal_name)`
/// lookups against the loaded `factionspawndatainfo.pabgb` + `.pabgh`.
#[repr(C)]
pub struct CrimsonFactionSpawnDataInfoHandle {
    by_key: HashMap<u32, String>,
    entries: Vec<(u32, String)>,
}

impl CrimsonFactionSpawnDataInfoHandle {
    fn from_bytes(pabgb: &[u8], pabgh: &[u8]) -> Self {
        let raw = parse_faction_spawn_data_info_lossy(pabgb, pabgh);
        ...
        CrimsonFactionSpawnDataInfoHandle { by_key, entries }
    }
}

// ── Load / free ────────────────────────────────────────────────────────────

/// Parse `factionspawndatainfo.pabgb` + `.pabgh` from disk.
///
/// # Safety
/// Both path arguments must be NUL-terminated UTF-8 strings. `out_handle`
/// must be non-null.
#[unsafe(no_mangle)]
pub unsafe extern "C" fn crimson_factionspawndatainfo_load_from_file(
    pabgb_path: *const c_char,
    pabgh_path: *const c_char,
    out_handle: *mut *mut CrimsonFactionSpawnDataInfoHandle,
) -> i32 {
    ...
    catch_unwind(AssertUnwindSafe(|| {
        ...
        let handle = CrimsonFactionSpawnDataInfoHandle::from_bytes(&pabgb, &pabgh);
...
/// Parse factionspawndatainfo pabgb + pabgh bytes already in memory.
///
/// # Safety
/// Both `pabgb` and `pabgh` must point to `*_len` readable bytes (may
/// be null iff length is 0). `out_handle` must be non-null.
#[unsafe(no_mangle)]
pub unsafe extern "C" fn crimson_factionspawndatainfo_load_from_bytes(
    pabgb: *const u8,
    pabgb_len: usize,
    pabgh: *const u8,
    pabgh_len: usize,
    ...
    catch_unwind(AssertUnwindSafe(|| {
        ...
        let handle =
            CrimsonFactionSpawnDataInfoHandle::from_bytes(pabgb_slice, pabgh_slice);
        unsafe { *out_handle = Box::into_raw(Box::new(handle)) };
        error::OK
    }))
    .unwrap_or(error::PANIC)
...
#[unsafe(no_mangle)]
pub unsafe extern "C" fn crimson_factionspawndatainfo_free(
    handle: *mut CrimsonFactionSpawnDataInfoHandle,
) {
    if handle.is_null() {
        return;
    }
...
/// Total number of `factionspawndatainfo` rows in the loaded table.
///
/// # Safety
/// `handle` must be live; `out_count` must be non-null.
#[unsafe(no_mangle)]
pub unsafe extern "C" fn crimson_factionspawndatainfo_entry_count(
    handle: *const CrimsonFactionSpawnDataInfoHandle,
    out_count: *mut u32,
) -> i32 {
...
/// Look up the internal name for a given `FactionSpawnDataKey (u32)`.
/// Two-call pattern.
///
/// Returns `NOT_FOUND` when `faction_spawn_data_key` isn't in the table.
///
/// **No `lookup_display_name` analogue** — see module docstring.
///
/// # Safety
/// `handle` and `required` must be non-null; `buf` may be null iff
/// `buf_len == 0`.
#[unsafe(no_mangle)]
pub unsafe extern "C" fn crimson_factionspawndatainfo_lookup_string_key(
    handle: *const CrimsonFactionSpawnDataInfoHandle,
    faction_spawn_data_key: u32,
    buf: *mut u8,
    buf_len: usize,
    required: *mut usize,
) -> i32 {
    ...
    catch_unwind(AssertUnwindSafe(|| {
        let h = unsafe { &*handle };
        let Some(name) = h.by_key.get(&faction_spawn_data_key) else {
...
/// Get the `(faction_spawn_data_key, internal_name)` pair at insertion
/// index `idx`. Two-call pattern.
///
/// # Safety
/// `handle`, `out_key`, and `required` must be non-null; `buf` may be
/// null iff `buf_len == 0`.
#[unsafe(no_mangle)]
pub unsafe extern "C" fn crimson_factionspawndatainfo_get_entry(
    handle: *const CrimsonFactionSpawnDataInfoHandle,
    idx: u32,
    out_key: *mut u32,
    buf: *mut u8,
...
mod tests {
    ...
    #[test]
    fn c_abi_factionspawndatainfo_live() {
        let Some(pamt_path) = find_pamt() else {
            eprintln!("skipping c_abi_factionspawndatainfo_live: no game install");
            return;
        };
        ...
        let rc = unsafe {
            crimson_factionspawndatainfo_load_from_bytes(
                pabgb.as_ptr(),
                pabgb.len(),
                pabgh.as_ptr(),
                pabgh.len(),
        ...
        let mut count: u32 = 0;
        assert_eq!(
            unsafe { crimson_factionspawndatainfo_entry_count(sh, &mut count) },
            error::OK
        );
        assert!(count > 100, "expected >100 factionspawn rows, got {count}");

        for &(key, expected) in KNOWN {
            let mut req: usize = 0;
            let rc_size = unsafe {
                crimson_factionspawndatainfo_lookup_string_key(
                    sh,
                    key,
                    ptr::null_mut(),
                    0,
            ...
            let got = read_string_result(rc_size, req, |b, n, r| unsafe {
                crimson_factionspawndatainfo_lookup_string_key(sh, key, b, n, r)
            });
            assert_eq!(got, expected, "FactionSpawnDataKey {key} name mismatch");
        }
        ...
        unsafe { crimson_factionspawndatainfo_free(sh) };
    }
}
